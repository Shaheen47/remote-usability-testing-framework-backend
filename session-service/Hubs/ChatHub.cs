...
namespace session_service.Hubs
{
    public class ChatHub : Hub, IChatHub
    {
        ...
        public async Task joinSession(string chatSessionId)
        {
            ...
            await Clients.Group(chatSessionId).SendAsync("userJoined",
                $"{Context.ConnectionId} has joined the group {chatSessionId}.");
        ...
        public async Task leaveSession(string chatSessionId)
        {
            ...
            await Clients.Group(chatSessionId)
                .SendAsync("userLeft", $"{Context.ConnectionId} has left the group {chatSessionId}.");
        }
